...
``ALL_TOOLS`` / ``MCP_TOOL_NAMES`` / ``builder.graph``. Each user's loaded
...
_user_runtimes: dict[str, UserDingTalkRuntime] = {}
# Dedup in-flight lazy loads per user (at most one subprocess spawn).
_loading_tasks: dict[str, asyncio.Task] = {}
# Guards the registry + store writes. Holds only SHORT critical sections —
# never across an ``await load_mcp_tools``, so concurrent users don't serialize.
...
def get_user_runtime(user_id: str) -> UserDingTalkRuntime | None:
    ...
    rt = _user_runtimes.get(user_id)
    if rt is None or not rt.enabled or not rt.tools:
        return None
    return rt


def get_user_tool_node(user_id: str) -> ToolNode | None:
...
async def _do_load(user_id: str, creds: dict) -> UserDingTalkRuntime:
    ...
    entry in ``_user_runtimes`` — never ``ALL_TOOLS`` / ``MCP_TOOL_NAMES``.
    """
...
async def enable_dingtalk(
    ...
    *,
    ...
    """
    existing = get_dingtalk_config(builder.store, user_id) or {}
    ...
    cur = _user_runtimes.get(user_id)
    ...
    try:
        rt = await _do_load(user_id, merged)
    except Exception as exc:
        async with _lock:
            rt = _user_runtimes.setdefault(
        ...
        if persist:
            put_dingtalk_config(builder.store, user_id, {**merged, "enabled": False})
...
async def disable_dingtalk(user_id: str, *, persist: bool = True) -> dict:
    ...
    """
    async with _lock:
        rt = _user_runtimes.setdefault(user_id, UserDingTalkRuntime(user_id=user_id))
        changed = rt.enabled
        rt.enabled = False
        rt.tools = []
        rt.tool_names = set()
        ...
        if persist:
            cfg = get_dingtalk_config(builder.store, user_id) or {}
            put_dingtalk_config(builder.store, user_id, {**cfg, "enabled": False})
    if changed:
...
async def mark_user_connected(user_id: str) -> None:
    ...
    async with _lock:
        ...
        rt.enabled = True
    cfg = get_dingtalk_config(builder.store, user_id) or {}
    put_dingtalk_config(builder.store, user_id, {**cfg, "enabled": True})
...
async def ensure_user_tools(user_id: str) -> UserDingTalkRuntime:
    ...
    while True:
        async with _lock:
            rt = _user_runtimes.get(user_id)
            # Registry hit only when the runtime is USABLE: exists, not mid-load,
            # and either has tools loaded, is disabled (nothing to load), or its
            # load already failed (don't re-spawn every turn). A runtime that is
            # enabled but has NO tools — e.g. OAuth callback marked it connected
            # via mark_user_connected(), which sets enabled but does not load the
            # tools — must fall through and load, or the user's DingTalk tools
            # never bind and AI answers "cannot query".
            if rt is not None and not rt.loading and (rt.tools or not rt.enabled or rt.load_failed):
                return rt
            task = _loading_tasks.get(user_id)
            if task is None:
                cfg = get_dingtalk_config(builder.store, user_id)
                ...
                task = asyncio.create_task(_do_load(user_id, rt.credentials))
                _loading_tasks[user_id] = task
        # Lock released before awaiting the subprocess load.
        try:
            result = await task
            async with _lock:
                _loading_tasks.pop(user_id, None)
            return result
        except Exception as exc:
            async with _lock:
                rt = _user_runtimes.get(user_id)
                if rt is not None:
                    ...
                    rt.last_error = str(exc)
                _loading_tasks.pop(user_id, None)
            return _user_runtimes.get(user_id) or UserDingTalkRuntime(user_id=user_id)
