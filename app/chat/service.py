...
async def chat_llm(
    ...
    ``session_id`` (generated by the frontend) becomes the LangGraph
    ``thread_id`` — it isolates conversation history. ``user_id`` only
    selects persisted store data.
...
async def resume_graph(
    ...
    turn — it is the LangGraph ``thread_id`` that holds the checkpoint.
